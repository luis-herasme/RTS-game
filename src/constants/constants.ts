...
export function isTextureDefined(type: string): boolean {
    const position: Vector | undefined = SPRITE_NAME_TO_SPRITE_POSITION_IN_TILE_MAP.get(type)
...
export function getTextureAtlasPosition(type: string): Vector {
    const position: Vector | undefined = SPRITE_NAME_TO_SPRITE_POSITION_IN_TILE_MAP.get(type)
    if (position === undefined) {
        throw Error("The texture for this type is not defined. Texture type: " + type)
    }
    return position
}

export const SPRITE_NAME_TO_SPRITE_POSITION_IN_TILE_MAP: Map<string, Vector> = new Map([
    ["W", new Vector(6, 4)],
    ["WBL", new Vector(0, 4)],
    ["WBR", new Vector(4, 4)],
    ["WB", new Vector(2, 4)],
