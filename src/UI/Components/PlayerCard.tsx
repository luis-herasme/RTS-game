...
export function PlayerCard({ name, color, validColors, removePlayer, setColor }) {
    ...
    return (
        <div style={{
            ...
            <span style={{ marginRight: "25px", marginLeft: "30px" }}>
                Name: {name},
            </span>
            Color:
                ...
                <select onChange={(e) => setColor(e.target.value)} style={{ padding: "0px", verticalAlign: "center" }}>
                    ...
                    {validColors.map(color => <option key={colorNames[color]} value={color}>{colorNames[color]}</option>)}
                </select>

            <span className="delete" onClick={removePlayer(name)} >x</span>
        </div>
    )
}
